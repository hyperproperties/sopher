...
func (generator TestCodeGenerator) Contract(function FunctionContract) *dst.BlockStmt {
	return dstx.Block(
		dstx.DefineS("test").As(dstx.Call(
			dstx.IndexS(function.ModelName()).Of(dstx.SelectS("NewAGHyperContractTest").FromS("sopher")),
		).Pass()),
		dstx.ExprStmt(
			dstx.Call(dstx.SelectS("Test").FromS("test")).Pass(
				dstx.Call(dstx.IndexS(function.ModelName()).Of(dstx.SelectS("Iterator").FromS("quick"))).Pass(),
				dstx.Ident(function.CallName()),
				dstx.Ident(function.ContractName()),
			),
		),
	)
}
...
func (generator TestCodeGenerator) Functions(file File) *dst.File {
	builder := dstx.FileS(file.pkg).
		Import(
			...
			dstx.AnonymousImportS("github.com/hyperproperties/sopher/pkg/quick"),
